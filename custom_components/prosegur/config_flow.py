...
from .const import CONF_COUNTRY, CONF_CONTRACT, DOMAIN
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_choose_contract(self, user_input=None):
        ...
        if user_input:
            ...
            return self.async_create_entry(title=f"Contract {user_input[CONF_CONTRACT]}", data=self.user_input)
        else:
            try:
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_CONTRACT): selector.selector(
                            {"select": {"options": [{"value": c['contractId'], "label": c['description']} for c in list_contracts]}}
                        ),
                }
            )
        )
